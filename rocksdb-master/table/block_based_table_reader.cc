...
#include "pmdk/pmdk_multiqueue.h"
...
Status BlockBasedTable::GetDataBlockFromCache(
  ...
  persistent_ptr<pmem_entry> aep_entry = nullptr;

  if(block_cache_compressed_handle == nullptr)
  {
      aep_entry = aep_read_cache->lookup(aep_pool,compressed_block_cache_key->data());
  }
  if(aep_entry != nullptr)
  {
	  //block->value = reinterpret_cast<Block*>(block_cache->Value(block->cache_handle));
	  BlockContents contents;
	  *contents_ = BlockContents(std::move(aep_entry->value->data()), aep_entry->value->size(), true,compression_type);
	  block->value = new Block(std::move(contents), port::kMaxUint64,read_amp_bytes_per_bit, ioptions.statistics);
	  return s;
  }
...
TBlockIter* BlockBasedTable::NewDataBlockIterator(
  ...
  if (s.ok()) {
	...
	{
		char cache_key[kMaxCacheKeyPrefixSize + kMaxVarint64Length];
		Slice key = GetCacheKey(rep->cache_key_prefix, rep->cache_key_prefix_size, handle, cache_key);
		aep_read_cache->push(pool,key->data(),block->data());
	}
